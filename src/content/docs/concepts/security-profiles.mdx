...
<Aside type="caution" title="Egress enforcement is strongest on Podman workers">
  The allowlist profiles (Standard and Strict) isolate DNS by creating the
  restricted network with Podman's `--disable-dns`, so a domain that isn't on the
  allowlist can't be resolved at all. Docker has no equivalent flag: on a
  Docker-backed worker the network keeps Docker's embedded resolver, so DNS and
  egress restrictions are **not enforced to the same standard** and a
  non-allowlisted domain may still resolve. If you rely on strict network
  isolation, run those DevBoxes on a Podman worker. Docker parity is on the
  roadmap.
</Aside>
...
- **[BYOM workers](/concepts/workers) only** — never [Cloud workers](/concepts/cloud-workers), which reject it unconditionally.
...
<Aside type="caution" title="Privileged mode removes all hardening">
  A privileged DevBox has root-equivalent access to the host worker's container
  engine, with every sandbox protection disabled. Use it only on machines you own
  and trust, and only when the workload genuinely needs to run containers inside the
  DevBox. See the [Privileged Mode guide](/guides/privileged-nested-containers) for
